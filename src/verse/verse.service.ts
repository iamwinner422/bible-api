...
import { BIBLE_APP_URL, DEFAULT_VERSIONS, SUPPORTED_LANGUAGES } from 'src/core/constants';
...
interface NextData {
  props: {
	pageProps: {
	  ...
	  chapterInfo?: { // Add chapterInfo as it's accessed
		content: string;
	  };
	  verses?: Array<{ // Add verses as it's accessed in the else branch
		content: string;
		reference: {
		  human: string;
		};
...
export class VerseService {
	getVersion(verseDto: GetVerseDto) {
		...
		try {
			...
			const foundedVersion = this.getVersionInfo(version, language) ?? DEFAULT_VERSIONS[language]
	...
	private async fetchVerses(url: string, book: string, chapter: string, verses: string) {
		...
		if(nextWay){
			...
			if (verses == "-1") {
				if (!jsonData.props.pageProps.chapterInfo?.content) {
					throw new InternalServerErrorException('Chapter content not found');
				}
				...
				if (!jsonData.props.pageProps.verses || jsonData.props.pageProps.verses.length === 0) {
					throw new InternalServerErrorException('Verse content not found');
				}
				const verse = jsonData.props.pageProps.verses[0].content;
				const reference = jsonData.props.pageProps.verses[0].reference.human;
